...
export class NotificationModuleAdapter {
    ...
    try {
      ...
      if (data.targetType === 'user' && data.userId) {
        ...
        if (campaign) {
          userIds = [
            campaign.businessId,
            ...campaign.applications.map((a: { influencerId: string }) => a.influencerId)
          ];
        }
      } else if (data.userId) {
        userIds = [data.userId];
   ...
   * 푸시 알림 전송 (구현 예정)
   */
  private async sendPushNotification(userIds: string[], _title: string, _message: string) {
    // TODO: FCM 또는 웹 푸시 구현
   ...
   * 이메일 알림 전송
   */
  private async sendEmailNotification(userIds: string[], data: any) {
    try {
      // 사용자 이메일 조회
      const users = await this.db.user.findMany({
        where: {
   ...
   * 알림 설정 조회
   */
  async getNotificationSettings(_userId: string) {
    try {
      // TODO: 사용자별 알림 설정 구현
      const settings = {
        email: {
