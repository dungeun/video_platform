...
class ResultValidator {
    ...
    validateError(error) {
        ...
        // 오류 코드 검증
        if (!error.code || !Object.values(types_1.VerificationErrorCode).includes(error.code)) {
            return false;
        }
        // 오류 메시지 검증
     ...
     */
    validateIntegrity(data, signature) {
     ...
     */
    validateTimestamp(timestamp, maxAgeMinutes = 10) {
        const now = new Date();
