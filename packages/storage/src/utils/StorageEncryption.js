export class StorageEncryption {
     ...
     * 비밀번호 변경 (재암호화)
     */
    async changePassword(encryptedData, oldPassword, newPassword) {
        // 기존 비밀번호로 복호화
        const decryptResult = await this.decrypt(encryptedData, oldPassword);
    ...
    async importKey(keyString) {
        try {
            ...
            const key = await this.crypto.subtle.importKey('raw', keyBuffer, {
                name: this.options.algorithm,
                length: this.options.keySize
     ...
     * HMAC 생성
     */
    async hmac(data, secret) {
        try {
            const key = await this.crypto.subtle.importKey('raw', this.textEncoder.encode(secret), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
    ...
    async deriveKey(password, salt) {
        // 비밀번호를 CryptoKey로 변환
        const passwordKey = await this.crypto.subtle.importKey('raw', this.textEncoder.encode(password), 'PBKDF2', false, ['deriveBits', 'deriveKey']);
        // PBKDF2로 키 유도
...
export class EncryptedStorageProvider {
    constructor(provider, password, options) {
    ...
    async get(key) {
        ...
        const decryptResult = await this.encryption.decrypt(result.data, this.password);
        if (decryptResult.isFailure) {
        ...
        try {
            ...
            for (const key of keysResult.data) {
                ...
                if (result.isSuccess && result.data !== null) {
                    const changeResult = await this.encryption.changePassword(result.data, this.password, newPassword);
