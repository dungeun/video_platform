export class StorageCompression {
    ...
    async decompress(compressedData) {
        try {
            ...
            if (typeof DecompressionStream !== 'undefined') {
                return this.decompressWithStream(compressedData);
            }
            // DecompressionStream이 없는 경우
            return this.decompressWithCustom(compressedData);
        }
        catch (error) {
     ...
     */
    async compressWithStream(data, originalSize) {
        try {
            const encoder = new TextEncoder();
            const input = encoder.encode(data);
            const compressionStream = new CompressionStream(this.options.algorithm);
    ...
    async decompressWithStream(compressedData) {
        try {
            ...
            const decompressionStream = new DecompressionStream(compressedData.algorithm);
     ...
     * 커스텀 압축 구현 (LZ 기반)
     */
    compressWithCustom(data, originalSize) {
        try {
     ...
     * 커스텀 압축 해제
     */
    decompressWithCustom(compressedData) {
        try {
     ...
     * 압축 통계
     */
    getCompressionStats(compressedData) {
        const ratio = this.calculateCompressionRatio(compressedData.originalSize, compressedData.compressedSize);
        return {
     ...
     * 최적 압축 알고리즘 선택
     */
    async selectBestAlgorithm(data) {
        ...
        for (const algorithm of algorithms) {
            ...
            if (result.isSuccess) {
                const ratio = this.calculateCompressionRatio(result.data.originalSize, result.data.compressedSize);
                if (ratio > bestRatio) {
                    bestRatio = ratio;
...
export class CompressedStorageProvider {
    ...
    async get(key) {
        ...
        if (result.data.compressed !== undefined) {
            const decompressResult = await this.compression.decompress(result.data);
...
//# sourceMappingURL=StorageCompression.js.map