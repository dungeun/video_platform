export class QuotaManager {
     ...
     * 쿼터 초과 시 자동 정리
     */
    async performAutoCleanup(provider, targetSize) {
     ...
     * 가장 오래된 항목부터 정리
     */
    async cleanupOldest(provider, targetBytes) {
     ...
     * 가장 큰 항목부터 정리
     */
    async cleanupLargest(provider, targetBytes) {
        const entriesResult = await provider.entries();
        if (entriesResult.isFailure)
            return 0;
        const entries = entriesResult.data
     ...
     */
    async cleanupLRU(provider, targetBytes) {
        const entriesResult = await provider.entries();
        if (entriesResult.isFailure)
            return 0;
        const entries = entriesResult.data
     ...
     * 쿼터 리스너 등록
     */
    addQuotaListener(listener) {
        this.quotaListeners.push(listener);
    }
    /**
     * 쿼터 리스너 제거
     */
    removeQuotaListener(listener) {
