...
export class StorageSync {
  ...
  async syncBidirectional(
    providerA: StorageProvider,
    providerB: StorageProvider
  ): Promise<Result<SyncResult>> {
    if (this.state.inProgress) {
      return Result.failure('SYNC_IN_PROGRESS', '동기화가 이미 진행 중입니다');
    }
    ...
    try {
      ...
      for (const key of allKeys) {
        try {
          ...
          const syncResult = await this.synchronizeItem(
            key,
            valueA.data,
            valueB.data,
            providerA,
   ...
   * 단일 항목 동기화
   */
  private async synchronizeItem(
    key: string,
    valueA: any,
    valueB: any,
    providerA: StorageProvider,
    providerB: StorageProvider
  ): Promise<Result<{ synced: boolean; conflict: boolean }>> {
    // 양쪽 모두 없는 경우
   ...
   * 충돌 해결
   */
  private async resolveConflict(key: string, valueA: any, valueB: any): Promise<any> {
   ...
   * 기본 충돌 해결자
   */
  private defaultConflictResolver(valueA: any, valueB: any, key: string): any {
    // 객체인 경우 얕은 병합
