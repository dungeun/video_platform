...
export class MemoryStorageProvider implements StorageProvider {
  public readonly name = 'MemoryStorage';
  ...
  public async get<T>(key: string): Promise<Result<T | null>> {
    try {
      ...
      return { success: true, data: entry.value.data as T };
  ...
  public async keys(query?: StorageQuery): Promise<Result<string[]>> {
    try {
      ...
      if (query?.tags && query.tags.length > 0) {
        keys = keys.filter(key => {
          const entry = this.storage.get(key);
          if (!entry || !entry.value.metadata?.tags) return false;
          
          return query.tags!.some(tag => 
            entry.value.metadata!.tags!.includes(tag)
          );
        });
      }
    ...
    try {
      ...
      for (const key of keysResult.data!) {
        ...
        if (entry && (!entry.value.metadata?.expires || Date.now() <= entry.value.metadata.expires)) {
          values.push(entry.value.data as T);
        }
      }
  ...
  public async entries<T>(query?: StorageQuery): Promise<Result<Array<[string, T]>>> {
    try {
      ...
      for (const key of keysResult.data!) {
        const entry = this.storage.get(key);
        if (entry && (!entry.value.metadata?.expires || Date.now() <= entry.value.metadata.expires)) {
          entries.push([key, entry.value.data as T]);
        }
      }
  ...
  private async evictEntries(requiredSize: number): Promise<void> {
    ...
    switch (this.config.evictionPolicy) {
      ...
      case EvictionPolicy.FIFO:
        // 가장 오래된 항목부터 제거
        entries.sort((a, b) => 
          (a[1].value.metadata?.created || 0) - (b[1].value.metadata?.created || 0)
        );
        break;
      ...
      case EvictionPolicy.RANDOM:
        ...
        for (let i = entries.length - 1; i > 0; i--) {
          ...
          [entries[i], entries[j]] = [entries[j] as [string, MemoryEntry], entries[i] as [string, MemoryEntry]];
        }
        break;
    }
