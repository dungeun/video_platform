...
export class PasswordManager {
  private readonly logger = new Logger('PasswordManager');
  private readonly saltRounds = 12;
  private readonly resetTokenExpiryHours = 2;
  ...
  async requestPasswordReset(input: PasswordResetInput): Promise<ServiceResponse<PasswordResetRequest>> {
    try {
      ...
      // Log security event
      await this.logSecurityEvent(user.id, SecurityEventType.PASSWORD_RESET_REQUESTED,
        `Password reset requested for email: ${validatedInput.email}`);
  ...
  async resetPassword(input: PasswordResetConfirmInput): Promise<ServiceResponse<boolean>> {
    try {
      ...
      try {
        ...
        // Log security event
        await this.logSecurityEvent(resetRequest.userId, SecurityEventType.PASSWORD_RESET_COMPLETED,
  ...
  async validatePasswordStrength(password: string): Promise<ServiceResponse<{ score: number; feedback: string[] }>> {
    try {
  ...
  private async isPasswordReused(userId: string, newPassword: string): Promise<boolean> {
    ...
    const history = await this.db.queryMany<{ password_hash: string }>(query, [userId, this.maxPasswordHistory]);
  ...
  private async savePasswordResetRequest(request: PasswordResetRequest): Promise<void> {
    const query = `
  ...
  private async findPasswordResetRequest(token: string): Promise<PasswordResetRequest | null> {
    const query = `SELECT * FROM password_reset_requests WHERE token = ?`;
    return await this.db.queryOne<PasswordResetRequest>(query, [token]);
  }

  private async deletePasswordResetRequest(id: string): Promise<void> {
    const query = `DELETE FROM password_reset_requests WHERE id = ?`;
    await this.db.execute(query, [id]);
  }

  private async cancelPendingResetRequests(userId: string): Promise<void> {
    const query = `DELETE FROM password_reset_requests WHERE user_id = ? AND used = false`;
    await this.db.execute(query, [userId]);
  }

  private async markResetRequestUsed(id: string): Promise<void> {
    const query = `
  ...
  private async resetLoginAttemptsAndUnlock(userId: string): Promise<void> {
    const query = `
      UPDATE user_accounts 
