...
export class UserAccountUpdater {
  private readonly logger = new Logger('UserAccountUpdater');

  constructor(private readonly db: DatabaseAdapter) {}

  async update(id: string, input: UpdateUserAccountInput): Promise<ServiceResponse<UserAccount>> {
    try {
      ...
      if (!updatedUser) {
        throw new UserAccountError(
          ...
          'Failed to retrieve updated user account'
        );
      }
  ...
  async incrementLoginAttempts(id: string): Promise<ServiceResponse<UserAccount>> {
    try {
      ...
      if (!updatedUser) {
        throw new UserAccountError(
          UserAccountErrorCode.NOT_FOUND,
