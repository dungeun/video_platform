 ...
 */
export function isAccountValid(account: UserAccount): boolean {
 ...
 */
export function canAccountLogin(account: UserAccount): boolean {
 ...
 */
export function isAccountTemporarilyLocked(account: UserAccount): boolean {
  if (!account.isLocked) return false;
  if (!account.lockExpiresAt) return true; // Permanent lock
 ...
 */
export function hasAccountLockExpired(account: UserAccount): boolean {
  if (!account.isLocked) return false;
  if (!account.lockExpiresAt) return false; // Permanent lock
  return new Date() >= account.lockExpiresAt;
}
 ...
 */
export function getAccountStatusText(account: UserAccount): string {
 ...
 */
export function getAccountStatusType(account: UserAccount): 'active' | 'inactive' | 'locked' | 'deleted' | 'unverified' {
  if (account.deletedAt) return 'deleted';
  if (account.isLocked) return 'locked';
  if (!account.isActive) return 'inactive';
  if (!account.emailVerified) return 'unverified';
  return 'active';
}
 ...
 */
export function hasExcessiveFailedAttempts(account: UserAccount, threshold: number = 5): boolean {
 ...
 */
export function getAccountAgeDays(account: UserAccount): number {
  const now = new Date();
 ...
 */
export function isNewAccount(account: UserAccount, days: number = 7): boolean {
 ...
 */
export function getTimeSinceLastLogin(account: UserAccount): string | null {
 ...
 */
export function getTimeUntilLockExpiry(account: UserAccount): string | null {
  if (!account.isLocked || !account.lockExpiresAt) return null;
 ...
 */
export function formatSecurityEventType(eventType: SecurityEventType): string {
  const eventTypeMap: Record<SecurityEventType, string> = {
 ...
 */
export function getSecurityEventSeverity(eventType: SecurityEventType): 'low' | 'medium' | 'high' {
 ...
 */
export function sanitizeAccountForDisplay(account: UserAccount): Omit<UserAccount, 'passwordHash'> {
 ...
 */
export function calculateAccountRiskScore(account: UserAccount): {
  score: number;
  factors: string[];
} {
  let score = 0;
  ...
  if (account.lastLoginAt) {
    const daysSinceLogin = Math.floor(
      (new Date().getTime() - new Date(account.lastLoginAt).getTime()) / (1000 * 60 * 60 * 24)
    );
