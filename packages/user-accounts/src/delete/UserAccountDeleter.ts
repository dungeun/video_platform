...
export class UserAccountDeleter {
  private readonly logger = new Logger('UserAccountDeleter');
  ...
  async restore(id: string): Promise<ServiceResponse<UserAccount>> {
    try {
      ...
      if (!restoredUser) {
        throw new UserAccountError(
          ...
          'Failed to retrieve restored user account'
        );
      }
  ...
  async hardDelete(id: string): Promise<ServiceResponse<boolean>> {
    try {
  ...
  async bulkSoftDelete(ids: string[]): Promise<ServiceResponse<{ deleted: number; failed: string[] }>> {
    try {
  ...
  async purgeOldDeleted(olderThanDays: number = 30): Promise<ServiceResponse<number>> {
    try {
      ...
      `;
      const accountsToPurge = await this.db.queryMany<{ id: string }>(accountsQuery, [cutoffDate]);
  ...
  private async findById(id: string): Promise<UserAccount | null> {
    const query = `
      SELECT * FROM user_accounts 
      WHERE id = ? AND deleted_at IS NULL
    `;
    
    const result = await this.db.queryOne<UserAccount>(query, [id]);
    return result || null;
  }

  private async findDeletedById(id: string): Promise<UserAccount | null> {
    const query = `
      SELECT * FROM user_accounts 
      WHERE id = ? AND deleted_at IS NOT NULL
    `;
  ...
  private async findByIdIncludingDeleted(id: string): Promise<UserAccount | null> {
    const query = `SELECT * FROM user_accounts WHERE id = ?`;
    const result = await this.db.queryOne<UserAccount>(query, [id]);
    return result || null;
  }

  private async checkEmailExistsInActive(email: string): Promise<boolean> {
    const query = `
