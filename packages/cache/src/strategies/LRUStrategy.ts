/**
 * @company/cache - LRU 캐시 전략
 * Least Recently Used 캐시 전략 구현
 */

import { CacheStrategy, CacheMetadata, CacheEntry } from '../types';
import { Logger } from '@company/core';

export class LRUStrategy<T = any> implements CacheStrategy<T> {
  ...
  constructor(maxSize: number = 100) {
    ...
    this.logger = new Logger('LRUStrategy');
  }
  ...
  public evict(): string | undefined {
    ...
    if (firstKey !== undefined) {
      ...
      this.logger.debug('LRU 항목 제거', { key: firstKey });
      return firstKey;
    }
