...
__export(index_exports, {
  CORE_MODULE_INFO: () => CORE_MODULE_INFO,
...
var Logger = class _Logger {
  ...
  writeLog(entry) {
    ...
    const module2 = `[${entry.module}]`.padEnd(20);
    let logMessage = `${timestamp} ${level} ${module2} ${entry.message}`;
...
var ModuleRegistry = class _ModuleRegistry {
   ...
   * 모듈 등록
   */
  async register(module2, dependencies = []) {
    const config = module2.getConfig();
    try {
