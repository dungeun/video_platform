...
export class ErrorHandler {
    ...
    normalizeError(error, context) {
        ...
        // 기타 타입인 경우
        return this.createError(CommonErrorCodes.SYSTEM_INTERNAL_ERROR, context || '알 수 없는 오류가 발생했습니다', { originalError: error });
    }
    /**
     * ModuleError 타입 가드
     */
    ...
    fromError(error, context) {
        ...
        if (this.isNetworkError(error)) {
            return this.createError(CommonErrorCodes.NETWORK_ERROR, '네트워크 연결에 실패했습니다', { originalMessage: error.message });
        }
        // 타임아웃 에러 판별
        if (this.isTimeoutError(error)) {
            return this.createError(CommonErrorCodes.SYSTEM_TIMEOUT, '요청 시간이 초과되었습니다', { originalMessage: error.message });
        }
        // 검증 에러 판별
        if (this.isValidationError(error)) {
            return this.createError(CommonErrorCodes.VALIDATION_FAILED, error.message || '입력값이 올바르지 않습니다', { originalMessage: error.message });
        }
        // 일반 에러
        return this.createError(CommonErrorCodes.SYSTEM_INTERNAL_ERROR, context || error.message || '시스템 오류가 발생했습니다', {
    ...
    fromString(error, context) {
        return this.createError(CommonErrorCodes.SYSTEM_INTERNAL_ERROR, context || error || '오류가 발생했습니다');
    }
    /**
     * 객체에서 ModuleError 생성
     */
