...
export class GeocodingService {
   ...
   * 주소를 좌표로 변환 (지오코딩)
   */
  async geocode(address: string | DetailedAddress): Promise<GeocodingResult | null> {
   ...
   * Kakao API를 사용한 지오코딩
   */
  private async geocodeWithKakao(address: string): Promise<GeocodingResult | null> {
    if (!this.kakaoApiKey) {
   ...
   */
  private async reverseGeocodeWithKakao(
  ...
  private async searchNearbyPlaces(
    coordinates: Coordinates, 
    radius: number = 500
  ): Promise<Array<{ name: string; distance: number; category: string; }>> {
    if (!this.kakaoApiKey) {
      return [];
    }
    ...
    try {
      const response = await fetch(
        `https://dapi.kakao.com/v2/local/search/category.json?x=${coordinates.x}&y=${coordinates.y}&radius=${radius}&sort=distance`,
        {
          headers: {
            'Authorization': `KakaoAK ${this.kakaoApiKey}`
          }
   ...
   * 정확도 판단
   */
  private determineAccuracy(doc: any): GeocodingResult['accuracy'] {
    if (doc.road_address) {
      return 'ROOFTOP';
    }
