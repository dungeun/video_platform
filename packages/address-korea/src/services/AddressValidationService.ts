...
export class AddressValidationService {
   ...
   * 주소 정규화
   */
  private normalizeAddress(address: DetailedAddress): DetailedAddress {
    return {
      ...address,
      // 우편번호 정규화 (하이픈 제거)
   ...
   * 시/군/구 정규화
   */
  private normalizeSigungu(sigungu: string): string {
    // 시/군/구가 누락된 경우 추가
  ...
  calculateCompleteness(address: DetailedAddress): number {
    ...
    // 필수 필드 (각 20점)
    if (address.zonecode) score += 20;
    if (address.address) score += 20;
    if (address.sido) score += 20;
    if (address.sigungu) score += 20;

    // 선택 필드 (각 10점)
    if (address.detailAddress) score += 10;
    if (address.addressType === 'ROAD' && address.roadname) score += 5;
    if (address.buildingName) score += 5;
