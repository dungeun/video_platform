...
export class AddressFormattingService {
   ...
   * 참고 항목 생성
   */
  private generateExtraAddress(result: AddressSearchResult): string {
    const extras: string[] = [];
   ...
   * 주소를 한 줄로 포맷팅
   */
  toSingleLine(address: DetailedAddress, includePostcode: boolean = true): string {
   ...
   * 주소를 여러 줄로 포맷팅
   */
  toMultiLine(address: DetailedAddress, includePostcode: boolean = true): string[] {
    const lines: string[] = [];
   ...
   * 약어 변환
   */
  private toShorthand(address: string): string {
   ...
   * 주소 일부 마스킹
   */
  maskAddress(address: DetailedAddress, maskDetails: boolean = true): string {
    const parts: string[] = [];
   ...
   * 주소 요약 (모바일용)
   */
  summarize(address: DetailedAddress, maxLength: number = 30): string {
