...
// ===== 테마 관리 =====
let currentTheme = { ...defaultFormTheme };
/**
 * 현재 테마 가져오기
 */
 ...
 * 크기별 스타일 가져오기
 */
export const getFormSizeStyle = (size = 'medium') => {
 ...
 * 변형별 스타일 가져오기
 */
export const getFormVariantStyle = (variant = 'default') => {
    const theme = getFormTheme();
    const baseStyle = {
 ...
 * 검증 상태별 스타일 가져오기
 */
export const getValidationStateStyle = (state) => {
    const theme = getFormTheme();
 ...
 * 라벨 스타일 가져오기
 */
export const getLabelStyle = (required) => {
    const theme = getFormTheme();
    return {
        display: 'block',
        fontSize: '0.875rem',
 ...
 * 전체 입력 스타일 가져오기
 */
export const getInputStyle = (size, variant, validationState, disabled, readOnly, focused) => {
 ...
 * CSS 클래스명 생성
 */
export const getFormClasses = (baseClass, size, variant, validationState, disabled, readOnly, focused, className) => {
    ...
    if (validationState)
        classes.push(`${baseClass}--${validationState}`);
    if (disabled)
