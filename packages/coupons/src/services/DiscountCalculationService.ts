...
export class DiscountCalculationService {
  calculateDiscount(
    ...
    switch (coupon.discountType) {
      ...
      case DiscountType.FREE_SHIPPING:
        discountAmount = this.calculateFreeShippingDiscount(
          context, 
          appliedRules
        );
        break;
  ...
  private getEligibleAmount(coupon: Coupon, context: ValidationContext): number {
    if (!context.products) return context.orderTotal;
  ...
  private isProductEligible(
    ...
    if (coupon.excludeProductIds?.includes(product.id)) return false;
    if (coupon.excludeCategoryIds?.includes(product.categoryId)) return false;

    // Check inclusions
    if (coupon.productIds && !coupon.productIds.includes(product.id)) return false;
    if (coupon.categoryIds && !coupon.categoryIds.includes(product.categoryId)) return false;
  ...
  calculateStackedDiscounts(
    ...
    const sortedCoupons = [...coupons].sort((a, b) => {
      if (a.discountType === DiscountType.PERCENTAGE && b.discountType !== DiscountType.PERCENTAGE) return -1;
      if (a.discountType !== DiscountType.PERCENTAGE && b.discountType === DiscountType.PERCENTAGE) return 1;
      return 0;
    });
