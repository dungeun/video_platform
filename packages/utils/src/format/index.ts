 ...
 * 파일 크기 포맷팅
 */
export function formatFileSize(bytes: number, decimals: number = 2): Result<string> {
  try {
 ...
 * 한국 전화번호 포맷팅
 */
export function formatKoreanPhoneNumber(phoneNumber: string): Result<string> {
  try {
 ...
 * 주민등록번호 포맷팅 (마스킹 옵션)
 */
export function formatKoreanSSN(ssn: string, mask: boolean = true): Result<string> {
  try {
 ...
 * 신용카드 번호 포맷팅 (마스킹 옵션)
 */
export function formatCreditCardNumber(cardNumber: string, mask: boolean = true): Result<string> {
  try {
 ...
 * 이메일 마스킹
 */
export function maskEmail(email: string): Result<string> {
  try {
 ...
 * 이름 마스킹
 */
export function maskName(name: string): Result<string> {
  try {
 ...
 * JSON 예쁘게 포맷팅
 */
export function formatJSON(obj: any, indent: number = 2): Result<string> {
  try {
    if (indent < 0) {
 ...
 * JSON 압축 (공백 제거)
 */
export function compactJSON(obj: any): Result<string> {
  try {
 ...
 * URL 쿼리 파라미터 포맷팅
 */
export function formatQueryParams(params: Record<string, any>): Result<string> {
  try {
 ...
 * URL 정규화
 */
export function normalizeUrl(url: string): Result<string> {
  try {
