 ...
 * 파일 또는 디렉토리 존재 확인
 */
export async function exists(filePath: string): Promise<Result<boolean>> {
  try {
 ...
 * 파일 존재 확인 (디렉토리 제외)
 */
export async function isFile(filePath: string): Promise<Result<boolean>> {
  try {
 ...
 * 디렉토리 존재 확인
 */
export async function isDirectory(dirPath: string): Promise<Result<boolean>> {
  try {
 ...
 * 텍스트 파일 읽기
 */
export async function readTextFile(filePath: string, encoding: BufferEncoding = 'utf8'): Promise<Result<string>> {
  try {
    if (typeof filePath !== 'string') {
      return { success: false, error: '파일 경로가 문자열이 아닙니다' };
    }
 ...
 * 바이너리 파일 읽기
 */
export async function readBinaryFile(filePath: string): Promise<Result<Buffer>> {
  try {
    if (typeof filePath !== 'string') {
      return { success: false, error: '파일 경로가 문자열이 아닙니다' };
    }
 ...
 * 바이너리 파일 쓰기
 */
export async function writeBinaryFile(filePath: string, buffer: Buffer): Promise<Result<void>> {
  try {
    if (typeof filePath !== 'string') {
      return { success: false, error: '파일 경로가 문자열이 아닙니다' };
    }
 ...
 * JSON 파일 쓰기
 */
export async function writeJsonFile(filePath: string, data: any, indent: number = 2): Promise<Result<void>> {
  try {
 ...
 * 파일 복사
 */
export async function copyFile(sourcePath: string, destPath: string): Promise<Result<void>> {
  try {
 ...
 * 파일 이동/이름변경
 */
export async function moveFile(sourcePath: string, destPath: string): Promise<Result<void>> {
  try {
    if (typeof sourcePath !== 'string' || typeof destPath !== 'string') {
      return { success: false, error: '경로가 문자열이 아닙니다' };
    }
 ...
 * 디렉토리 생성 (재귀적)
 */
export async function createDirectory(dirPath: string): Promise<Result<void>> {
  try {
 ...
 * 디렉토리 내용 읽기
 */
export async function readDirectory(dirPath: string): Promise<Result<string[]>> {
  try {
    if (typeof dirPath !== 'string') {
      return { success: false, error: '디렉토리 경로가 문자열이 아닙니다' };
    }
 ...
 * 디렉토리 삭제 (재귀적)
 */
export async function deleteDirectory(dirPath: string): Promise<Result<void>> {
  try {
    if (typeof dirPath !== 'string') {
      return { success: false, error: '디렉토리 경로가 문자열이 아닙니다' };
    }
 ...
 * 파일 정보 가져오기
 */
export async function getFileInfo(filePath: string): Promise<Result<FileInfo>> {
  try {
    if (typeof filePath !== 'string') {
      return { success: false, error: '파일 경로가 문자열이 아닙니다' };
    }
 ...
 * 파일 크기 가져오기
 */
export async function getFileSize(filePath: string): Promise<Result<number>> {
  try {
 ...
 * 절대 경로로 변환
 */
export function getAbsolutePath(filePath: string): Result<string> {
  try {
    if (typeof filePath !== 'string') {
      return { success: false, error: '파일 경로가 문자열이 아닙니다' };
    }
 ...
 * 상대 경로 계산
 */
export function getRelativePath(from: string, to: string): Result<string> {
  try {
 ...
 * 경로 정규화
 */
export function normalizePath(filePath: string): Result<string> {
  try {
    if (typeof filePath !== 'string') {
      return { success: false, error: '파일 경로가 문자열이 아닙니다' };
    }
 ...
 * 경로 조인
 */
export function joinPath(...segments: string[]): Result<string> {
  try {
 ...
 * 안전한 파일명 생성
 */
export function sanitizeFileName(fileName: string): Result<string> {
  try {
 ...
 * 고유한 파일명 생성
 */
export async function generateUniqueFileName(dirPath: string, baseName: string): Promise<Result<string>> {
  try {
