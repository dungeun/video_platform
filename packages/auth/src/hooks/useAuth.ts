 ...
 */
export function useAuth(authService?: AuthService): UseAuthReturn {
  const {
    user,
    session,
    status,
  ...
  const [internalAuthService, setInternalAuthService] = useState<AuthService | null>(authService || null);
  ...
  const logout = useCallback(async (): Promise<AuthResult<LogoutResponse>> => {
    if (!currentAuthService) {
      return { success: false, error: '인증 서비스가 초기화되지 않았습니다' };
    }
