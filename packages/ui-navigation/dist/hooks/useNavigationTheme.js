...
import { getNavigationTheme, setNavigationTheme, defaultNavigationTheme, darkNavigationTheme, getThemeStyles } from '../utils/navigationTheme';
const STORAGE_KEY = 'navigation-theme';
const DARK_MODE_KEY = 'navigation-dark-mode';
export const useNavigationTheme = (options = {}) => {
    const { defaultTheme, autoDetectDarkMode = true, persistTheme = true, storageKey = STORAGE_KEY } = options;
    ...
    // DOM 요소에 테마 적용
    const applyTheme = useCallback((element) => {
    ...
    // DOM 요소에서 테마 제거
    const removeTheme = useCallback((element) => {
        const target = element || document.documentElement;
        // CSS 변수 제거
        const properties = [
            '--nav-color-primary',
...
//# sourceMappingURL=useNavigationTheme.js.map