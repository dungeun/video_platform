...
// 현재 테마 상태
let currentTheme: NavigationTheme = defaultNavigationTheme;
 ...
 * 네비게이션 테마 설정
 */
export const setNavigationTheme = (theme: Partial<NavigationTheme>): void => {
  currentTheme = {
    ...currentTheme,
    ...theme,
 ...
 * 다크 테마로 전환
 */
export const enableDarkTheme = (): void => {
 ...
 * 라이트 테마로 전환
 */
export const enableLightTheme = (): void => {
 ...
 * 테마 기반 스타일 생성
 */
export const getThemeStyles = (theme: NavigationTheme = currentTheme) => ({
  // 공통 스타일
  base: {
 ...
 * CSS 변수로 테마 적용
 */
export const applyThemeVariables = (theme: NavigationTheme = currentTheme): string => {
  return `
