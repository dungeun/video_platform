...
export const flattenNavigationItems = (items: NavigationItem[]): NavigationItem[] => {
  const flatten = (items: NavigationItem[], depth = 0): NavigationItem[] => {
    const result: NavigationItem[] = [];
    items.forEach(item => {
      result.push({ ...item, depth } as NavigationItem & { depth: number });
      if (item.children) {
...
export const generateBreadcrumbs = (
  ...
  const findPath = (items: NavigationItem[], path: NavigationItem[]): boolean => {
    for (const item of items) {
...
export const filterNavigationItems = (
  ...
  const searchItems = (items: NavigationItem[]): NavigationItem[] => {
 ...
 * 네비게이션 아이템 깊이 계산
 */
export const getNavigationDepth = (items: NavigationItem[]): number => {
  let maxDepth = 0;
  
  const calculateDepth = (items: NavigationItem[], currentDepth = 1): void => {
 ...
 * 네비게이션 아이템 그룹핑
 */
export const groupNavigationItems = (
  items: NavigationItem[],
