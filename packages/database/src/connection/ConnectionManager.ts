...
export class ConnectionManager {
  ...
  private async createConnection(config: DatabaseConfig, connectionId: string): Promise<any> {
    ...
    try {
      ...
      throw new ConnectionError(
        `${config.provider} 연결 생성 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`,
        config.provider
      );
    }
  }

  private buildKnexConfig(config: DatabaseConfig): any {
    const baseConfig = {
  ...
  private getKnexClient(provider: DatabaseProvider): string {
    switch (provider) {
      case 'postgresql':
        return 'pg';
      case 'mysql':
  ...
  private async testConnection(connection: any, provider: DatabaseProvider): Promise<void> {
    try {
      // 간단한 쿼리로 연결 테스트
      switch (provider) {
        case 'postgresql':
      ...
      throw new ConnectionError(
        `연결 테스트 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`,
        provider
      );
    }
  }

  private setupConnectionEvents(connection: any, connectionId: string): void {
    // Query 이벤트
    connection.on('query', (queryData: any) => {
    ...
    connection.on('query-response', (response: any, queryData: any) => {
    ...
    connection.on('query-error', (error: any, queryData: any) => {
  ...
  private async disconnectSingle(connectionId: string): Promise<DatabaseResult> {
    const connection = this.connections.get(connectionId);
    if (!connection) {
      return {
        success: false,
  ...
  private async disconnectAll(): Promise<DatabaseResult> {
