...
export class PostgreSQLProvider {
    constructor(config) {
        this.config = config;
        this.logger = new Logger('PostgreSQLProvider');
     ...
     * JSONB 쿼리 헬퍼
     */
    buildJsonbQuery(column, path, operator, value) {
     ...
     * 전문 검색 쿼리 헬퍼
     */
    buildFullTextSearch(column, query, language = 'simple') {
        return `to_tsvector('${language}', ${column}) @@ plainto_tsquery('${language}', '${query}')`;
    }
}
//# sourceMappingURL=PostgreSQLProvider.js.map