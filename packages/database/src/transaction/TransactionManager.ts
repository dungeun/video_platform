...
export class DatabaseTransaction implements Transaction {
  ...
  public readonly queries: QueryMetrics[] = [];
  
  private logger = new Logger('Transaction');
  private knexTransaction: any;
  private active = true;
...
export class TransactionManager {
   ...
   * 새 트랜잭션 시작
   */
  async begin(): Promise<{ success: boolean; transaction?: DatabaseTransaction; error?: string }> {
    try {
  ...
  ): Promise<{ success: boolean; data?: T; error?: string; transactionId?: string }> {
