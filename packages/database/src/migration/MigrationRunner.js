...
export class MigrationRunner {
        ...
        try {
            ...
            for (const migration of pendingMigrations) {
                try {
                    ...
                    throw new MigrationError(`마이그레이션 실행 실패: ${migration.name} - ${errorMessage}`, migration.name);
                }
            }
        ...
        try {
            ...
            for (const record of toRollback) {
                try {
                    ...
                    throw new MigrationError(`마이그레이션 롤백 실패: ${record.name} - ${errorMessage}`, record.name);
                }
            }
            const duration = Date.now() - startTime;
    ...
    async executeMigration(migration, direction) {
        ...
        try {
            ...
            throw new MigrationError(`마이그레이션 ${direction.toUpperCase()} 실패: ${migration.name} - ${errorMessage}`, migration.name);
        }
    }
    async addMigrationRecord(migration) {
