...
export interface DatabaseEvents {
  'connection:created': { connectionId: string; config: DatabaseConfig };
  'connection:destroyed': { connectionId: string };
  'query:start': { query: string; bindings: any[]; connectionId: string };
  'query:complete': { query: string; bindings: any[]; duration: number; connectionId: string };
  'query:error': { query: string; bindings: any[]; error: string; connectionId: string };
  'transaction:start': { transactionId: string; connectionId: string };
  'transaction:commit': { transactionId: string; duration: number; connectionId: string };
  'transaction:rollback': { transactionId: string; duration: number; connectionId: string };
  'migration:start': { migration: string };
  'migration:complete': { migration: string; duration: number };
  'migration:error': { migration: string; error: string };
}
...
export class ConnectionError extends DatabaseError {
  constructor(message: string, public provider?: DatabaseProvider) {
...
export class MigrationError extends DatabaseError {
  constructor(message: string, public migrationName?: string) {
