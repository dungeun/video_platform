...
exports.CampaignModuleAdapter = void 0;
class CampaignModuleAdapter {
    db;
    redis;
    eventBus;
    constructor(deps) {
    ...
    async createCampaign(businessId, data) {
        try {
            ...
            // 캐시 저장
            await this.redis.set(`campaign:${campaign.id}`, JSON.stringify(campaign), 3600 // 1시간
            );
            // 이벤트 발행
    ...
    async getCampaignDetail(campaignId) {
        try {
            ...
            // 캐시 저장
            await this.redis.setWithExpiry(`campaign:${campaignId}`, JSON.stringify(campaign), 3600);
            return {
                success: true,
                data: campaign
            };
    ...
    async applyCampaign(campaignId, influencerId, message) {
        try {
            ...
            // 팔로워 수 확인
            if (influencer.profile.followerCount < campaign.target.minFollowers ||
                influencer.profile.followerCount > campaign.target.maxFollowers) {
                return {
                    success: false,
    ...
    async getCampaignStats(campaignId, businessId) {
        try {
            ...
            // 통계 집계
            const [applications, approved, completed, contents] = await Promise.all([
                // 전체 지원자
...
exports.CampaignModuleAdapter = CampaignModuleAdapter;
