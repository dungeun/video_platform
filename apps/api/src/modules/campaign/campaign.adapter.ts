...
export class CampaignModuleAdapter {
  ...
  async applyCampaign(campaignId: string, influencerId: string, message?: string) {
    try {
      ...
      // 팔로워 수 확인
      if (
        influencer.profile.followerCount < campaign.target!.minFollowers ||
  ...
  async getCampaignStats(campaignId: string, businessId: string) {
    try {
      ...
      // 성과 집계
      const totalReach = contents.reduce((sum: number, c: { metrics?: { reach?: number } }) => sum + (c.metrics?.reach || 0), 0);
      const totalEngagement = contents.reduce((sum: number, c: { metrics?: { engagement?: number } }) => sum + (c.metrics?.engagement || 0), 0);
      const totalConversions = contents.reduce((sum: number, c: { metrics?: { conversions?: number } }) => sum + (c.metrics?.conversions || 0), 0);
